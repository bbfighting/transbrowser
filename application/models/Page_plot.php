...
class Page_plot extends CI_Model {
    ...
    public function run($ri_track)
    {
        ...
        if ($this->_cor2 == -1)
        {
            ...
            imagefilledrectangle($im, $word_AUG_x, $head_tail_y1, $bar_AUG_X2, $cor_y2, $color_Text);
...
////////////////////////////////////////////////////////////////////////////////////////
            ...
            if ($bar_uorf_y2 < $bar_uorf_word_y + $diff_bar_content)
                ...
                $bar_uorf_word_y = $bar_uorf_y2 - ($bar_uorf_y2 - $bar_uorf_y1 - strlen("IRES") * $len_word_up_height) / 2;
        ...
        $bar_IRES_word_y = $IRES_bara_y1 + strlen("IRES") * $len_word_up_height;
        $IRES_count = 0;

////////////////////////////////////////////////////////////////////////////////////////
        $IRES_level = array();
        $level = 0;        
        $temp_level_array = array();
////////////////////////////////////////////////////////////////////////////////////////
...
/***************************************************************************************/                   
                    ...
                    $temp_leve_array[$level] = $compare_line;
...
////////////////////////////////////////////////////////////////////////////////////////
                    ...
                    for ($i = $temp_key1; $i <= $temp_key2; $i++)
                    {
                        ...
                        if (! array_key_exists($level_key, $IRES_level))
                            $IRES_level[$level_key] = array($start_value . ";" . $end_value . ";" . ($level + 1) . ";" . $IRES_count);
                        else
                            array_push($IRES_level[$level_key], $start_value . ";" . $end_value . ";" . ($level + 1) . ";" . $IRES_count);
                    }
////////////////////////////////////////////////////////////////////////////////////////
                }
            }
        ...
        if ($bar_IRES_y2 < $bar_IRES_word_y + $diff_bar_content)
            ...
            $bar_IRES_word_y = $bar_IRES_y2 - ($bar_IRES_y2 - $bar_IRES_y1 - strlen("IRES") * $len_word_up_height) / 2;
        imagefilledrectangle($im, $bar_x, $bar_IRES_y1, $cor_x1, $bar_IRES_y2, $color_bar2b); 
        imageStringUp($im, 5, $bar_x, $bar_IRES_word_y, "IRES", $background);   
        $temp_y = $bar_IRES_y2;
        // IRES end --------------------------------------------------------
        ...
        if ($this->_issearch)
        {
            ...
            $group2_target = false;
            $group3_target = false;
            $ri_group1 = array('G1_1', 'G1_2', 'S1', 'S2', 'M1', 'M2');
            $ri_group2 = array('H_Rep1', 'H_Rep2', 'N_Rep1', 'N_Rep2');
            $ri_group3 = array('Ribo_GFP', 'Ribo_GFPQ1', 'Ribo_nEGF', 'Ribo_pEGF');
            $bar_ri_y1 = $temp_y + $diff_space_bar;
            $space_track = 50;
            $space_ri_max = 35;
            $ri_depth_y1 = $bar_ri_y1 + $diff_bar_content + $space_track;
            $bar_ri_word_y = $bar_ri_y1 + $diff_bar_content + strlen("Ribosome Profiling Data") * $len_word_up_height;

            if ($ri_track != "no")
            {
                $temp_ri_max = 0;
                $track_notNone_array = array();
                $trans_array = explode("A", $ri_track);
                $trans_word_x = $cor_x2 + $len_word_space;

                $ri_depth_array_start = $this->_cor1 + $row_seq_len;
                $ri_depth_array_end = $seq_len - 1;


                $sql = sprintf("SELECT b.* FROM saa_acc_id_%s a JOIN saa_ri_dep_%s b ON a.id = b.acc_id WHERE a.acc LIKE '%s'", $this->_species, $this->_species, $this->_asname);
                $query = $this->db->query($sql);

                $temp_depth_array = array(0);
                foreach ($trans_array as $sub_trans)
                {
                    $temp_depth_str = "";

                    if (in_array($sub_trans, $ri_group1) && !$group1_target)
                    {
                        $group1_target = true;
                        $temp_depth_array = array();
                    }
                    if (in_array($sub_trans, $ri_group2) && !$group2_target)
                    {
                        $group2_target = true;
                        $temp_depth_array = array();
                    }
                    if (in_array($sub_trans, $ri_group3) && !$group3_target)
                    {
                        $group3_target = true;
                        $temp_depth_array = array();
                    }
                    ...
                    if ($temp_depth_str != "")
                    {
                        ...
                        $temp_depth_array = array(max(array_map('floatval', $temp_depth_array)));
                        array_push($track_notNone_array, $sub_trans);
                        $temp_ri_max = $temp_depth_array[0];
                    }
                    if (in_array($sub_trans, $ri_group1))
                    {
                        $ri_gsm_max = $temp_ri_max;
                    }
                    if (in_array($sub_trans, $ri_group2))
                    {
                        $ri_hn_max = $temp_ri_max;
                    }
                    if (in_array($sub_trans, $ri_group3))
                    {
                        $ri_ribo_max = $temp_ri_max;
                    }
                }
                ...
                foreach ($trans_array as $sub_trans)
                {
                    ...
                    if (isset($ri_gsm_max) && in_array($sub_trans, $ri_group1))
                            $ri_max = $ri_gsm_max;
                    else if (isset($ri_hn_max) && in_array($sub_trans, $ri_group2))
                            $ri_max = $ri_hn_max;
                    else if(isset($ri_ribo_max) && in_array($sub_trans, $ri_group3))
                            $ri_max = $ri_ribo_max;
                    else
                        $ri_max = 0;

                    $range_dep = ($ri_max == 0) ? 0 : $space_ri_max / $ri_max;
                    // ri_dep_cor_start
                    if ($ri_max != 0)
                    {
                        for ($h = 0; $h <= 10; $h ++)
                    ...
                    if (in_array($sub_trans, $track_notNone_array))
                    {
                        ...
                        imagefttext($im, $font_size, 0, $trans_word_x - strlen($sub_trans) * $len_word_width, $ri_depth_y1 + $len_word_height * 2, $color_Text, $font_file, $sub_trans);
            ...
            if ($bar_ri_y2 < $bar_ri_word_y)
                ...
                $bar_ri_word_y = $bar_ri_y2 - ($bar_ri_y2 - $bar_ri_y1 - strlen("Ribosome Profiling Data") * $len_word_up_height) / 2;
            imagefilledrectangle($im, $bar_x, $bar_ri_y1, $cor_x1, $bar_ri_y2, $color_bar1b); 
            imageStringUp($im, 5, $bar_x, $bar_ri_word_y, "Ribosome Profiling Data", $background);
            $temp_y = $bar_ri_y2;
        }
        // ribosome end --------------------------------------------------------
...
////////////////////////////////////////////////////////////////////////////////////////
        ...
        if ($this->_issearch && ($IRES_all_str || $row->uorf))
        {
            if ($IRES_all_str != "")
            {
                imagefilledrectangle($im, 0, $temp_y, 50, $temp_y + 10, $color_bar2a);
                imagefttext($im, $font_size, 0, 55, $temp_y + $len_word_height, $color_Text, $font_file, "IRES");
                $temp_y = $temp_y + 20;
            }
            if ($row->uorf != NULL)
            {
                imagefilledrectangle($im, 0, $temp_y, 50, $temp_y + 10, $color_bar3a);
                imagefttext($im, $font_size, 0, 55, $temp_y + $len_word_height, $color_Text, $font_file, "uORF");
                $temp_y = $temp_y + 20;
            }
            $temp_y = $temp_y + 80;
        }       
